...
1. Конкатенируем название и все свойства товара.
2. Отдаем эту строчку в MongoDB - перед поиском он автоматически производит лемматизацию.
3. Затем  производится полнотекстовый поиск по этой строке по имеющимся эталонам, у которых так же предварительно сконкатенированны название и свойства.
4. После мы выбираем эталон с наибольшим textScore и возвращаем его id.
...
3. Очистили данные от специальных символов

Такое действие позволяет нам уменьшить общее количество токенов, уменьшить количество текста, по которому выполняется поиск. Использование этого подхода увеличило точность нашей модели на 0.02 и слегка (на 12%) увелилась скорость по сравнению с предыдущим успешным вариантом.
Данные по бенчмарчку можно увидеть ниже.
...
**Чтобы у вас была возможность протестировать производительность и убедиться в легитимности тестов, мы рассказываем, как вы можете запустить наш тест на своей машине в секции** [Как тестировали наше решение мы](#Как-тестировали-наше-решение-мы)
...
make run
```
После этого ваш сервис станет доступен на `localhost:8100`, если вы не меняли порт в `.env`, а на `localhost:8100/docs` будет располагаться  интерактивная документация по API.
...
```
### Как тестировали наше решение мы

1. Запустите сервис согласно [инструкции по запуску](#Как-запустить)

```shell
$ make db && make run
```

2. Откройте web-интерфейс, `localhost:8100/docs`

3. Найдите метод `/user/register, нажмите на него`, затем нажмите на `"Try it out"` и зарегистрируйте пользователя-продюсера. Чтобы  это сделать, введите username и password, и нажмите execute.

![image](https://user-images.githubusercontent.com/53406289/185787078-00d4e89a-a5c7-4906-b2a9-f1e7ffd7a24f.png)

4. Авторизируйтесь в web-интерфейсе: для этого нажмите `Authorize` в правом верхнем углу и введите username и password от аккаунта продюсера.

![image](https://user-images.githubusercontent.com/53406289/185787141-24c8e74a-66ff-438c-bad6-0fb9de48ac2c.png)

![image](https://user-images.githubusercontent.com/53406289/185787153-5b9eb2fb-0df6-4ca7-b14c-21e2758d158e.png)

5. Загрузите тестовые данные. В качестве тестовых данных мы использовали товары, которые не являются эталонами из предоставленного датасета ( файл `tests/test_products.json`). Чтобы загрузить данные, найдите метод `/producer/upload/file`, нажмите на него, нажмите на `"Try it out"` и загрузите json-файл с тестовыми данными. Нажмите execute.
