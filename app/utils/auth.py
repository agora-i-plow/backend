...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None) -> str:
    ...
    encoded_jwt = jwt.encode(to_encode, settings.FASTAPI_SECRET, algorithm=settings.FASTAPI_HASH_ALGORITHM)
    return encoded_jwt

def verify_token(token: str, credentails_exception) -> str:
    settings = get_fastapi_settings()
    try:
        playload = jwt.decode(token, settings.FASTAPI_SECRET, algorithms=[settings.FASTAPI_HASH_ALGORITHM])
